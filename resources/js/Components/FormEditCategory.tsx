...
export default function FormEditCategory({
  ...
  const [billboardImagePreview, setBillboardImagePreview] = useState(category.billboard ?? "");
  ...
  const handleFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      setData("billboard", file);
      reader.onload = (e) => {
        setBillboardImagePreview(e.target?.result as string);
      }
        ...
        <div className="flex flex-col gap-y-4">
          ...
          <div className="grid grid-cols-2 gap-x-4">
            ...
            <div className="col-span-2 flex flex-col gap-y-2">
              ...
              <input id="billboard" type="file" onChange={handleFileChange} className="hidden" />
              {errors.billboard && (
                <div className="flex items-center gap-x-1 text-red-500 text-sm font-normal ml-1">
                  <XCircle size={20} className="stroke-[1]" />
                  {errors.billboard}
